...
namespace Sign.Core
{
    internal class CabContainer : Container
    {
        ...
        private readonly List<string> _entryNames = new();
        ...
        public override ValueTask OpenAsync()
        {
            ...
            CabInfo cab = new(_cabFile.FullName);

            // Windows Installer requires the order of the files in a cabinet to match the order of
            // the File table's Sequence column, so remember it for SaveAsync.
            // See https://learn.microsoft.com/windows/win32/msi/cabinet-files.
            _entryNames.AddRange(cab.GetFiles().Select(file => Path.Combine(file.Path, file.Name)));

            cab.Unpack(TemporaryDirectory.Directory.FullName);
        ...
        public override ValueTask SaveAsync()
        {
            ...
            using (TemporaryDirectory temporaryDirectory = new(_directoryService))
            {
                ...
                new CabInfo(destinationFilePath).PackFiles(
                    TemporaryDirectory.Directory.FullName,
                    _entryNames,
                    _entryNames,
