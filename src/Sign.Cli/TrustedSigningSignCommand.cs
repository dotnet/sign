...
namespace Sign.Cli
{
    internal sealed class TrustedSigningSignCommand : TrustedSigningCommand
    {
        internal Argument<string?> FileArgument { get; } = new("file(s)", Resources.FilesArgumentDescription);

        internal TrustedSigningSignCommand(CodeCommand codeCommand, IServiceProviderFactory serviceProviderFactory)
        {
