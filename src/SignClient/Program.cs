        ...
        static async Task<int> DoMain(string[] args)
        {
            try
            {
                ...
                ArgumentSyntax.Parse(args, syntax =>
                {
                    ...
                    syntax.DefineOption("h|hashmode", ref hashMode, s => (HashMode)Enum.Parse(typeof(HashMode), s, true), "Hash mode: either dual or Sha256. Default is Sha256. Dual signs with both Sha-1 and Sha-256 for files that support it. For files that don't support dual, Sha-256 is used. Sha1 is only supported for VSIX files as VS before 2015 doesn't support Sha256.");
