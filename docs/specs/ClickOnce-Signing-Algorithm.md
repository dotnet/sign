...
1. Before resolving files, require exactly one assembly dependency and no file references. The sole assembly dependency must be [`DeployManifest.EntryPoint`](https://learn.microsoft.com/dotnet/api/microsoft.build.tasks.deployment.manifestutilities.deploymanifest.entrypoint?view=msbuild-17-netcore), must represent the application manifest, and must not be a prerequisite, optional, or resource-fallback dependency. Inspect the preserved manifest XML for resource-fallback attributes because `ManifestUtilities` does not expose or preserve them in its object model. Fail with a clear error and suggest `--clickonce-signing-version 2 --no-update-clickonce-manifest` if the reference structure does not satisfy these ClickOnce runtime requirements. This fail-fast validation also prevents `ManifestUtilities` or `mage.exe` from reordering or rewriting unsupported references before signing.
...
1. Identify each application-manifest `AssemblyReference` and `FileReference` that represents a physical file published with the application, following the file-extension mapping rules above and searching the application manifest's directory first, then the deployment manifest's directory as a fallback (if different). Use these Sign CLI lookup rules directly for source discovery rather than calling `ApplicationManifest.ResolveFiles(...)`, whose lookup semantics do not account for file-extension mapping or deployment-directory fallback. A failed intermediate candidate probe is not a diagnostic when a later allowed candidate resolves the reference. Fail with a clear error if no allowed candidate locates a required file.
1. Ensure staged files preserve their manifest-relative paths without writing outside the staging directory. If Sign CLI cannot represent a referenced layout within staging, fail before copying files and report that the layout is unsupported.
...
1. Resolve the application manifest's references against the staged application directory using `ApplicationManifest.ResolveFiles(string[])`. Log every newly produced `OutputMessage`. Warnings are non-fatal; fail without signing any payload or updating or signing the application manifest if any newly produced message is an error. Independently verify that every required physical reference's `ResolvedPath` identifies its corresponding suffix-stripped staged file and remains within the staging directory; resolution to a different file does not satisfy this postcondition.
...
1. Immediately before calling `ApplicationManifest.UpdateFileInfo("v4.5")`, verify again that every required physical reference's `ResolvedPath` identifies its expected suffix-stripped staged file, that the file exists, and that it remains within the staging directory. Then refresh payload SHA-256 hashes and sizes and restore the mapping-added suffixes. (`UpdateFileInfo(string)` hashes each reference's current `ResolvedPath`.) For each payload that was signed, make its result available and complete its coordinated signing operation. Then sign the application manifest, make its signed result available, and complete its operation.
...
1. Identify each `AssemblyReference` and `FileReference` that represents a physical file published with the application. Look for its manifest target path relative to the application manifest's directory first; if that path does not exist, look for the same path with one additional `.deploy` suffix and record that suffix as mapping-added. Use these Sign CLI lookup rules directly for source discovery rather than calling `ApplicationManifest.ResolveFiles(...)`, whose lookup semantics do not account for file-extension mapping. A failed exact-path probe is not a diagnostic when the mapped candidate resolves the reference. Fail with a clear error if neither candidate locates a required file.
1. Ensure staged files preserve their manifest-relative paths without writing outside the staging directory. If Sign CLI cannot represent a referenced layout within staging, fail before copying files and report that the layout is unsupported.
...
1. Resolve the application manifest's references against the staged application directory using `ApplicationManifest.ResolveFiles(string[])`. Log every newly produced `OutputMessage`. Warnings are non-fatal; fail without signing any payload or updating or signing the application manifest if any newly produced message is an error. Independently verify that every required physical reference's `ResolvedPath` identifies its corresponding suffix-stripped staged file and remains within the staging directory; resolution to a different file does not satisfy this postcondition.
1. Apply the standard signing behavior only to payloads selected by the user's file-matching patterns; leave excluded or unsupported payloads unchanged. Include every referenced payload when refreshing application-manifest metadata.
1. Immediately before calling `ApplicationManifest.UpdateFileInfo("v4.5")`, verify again that every required physical reference's `ResolvedPath` identifies its expected suffix-stripped staged file, that the file exists, and that it remains within the staging directory. Then update the file information and restore the mapping-added suffixes. For each payload that was signed, make its result available and complete its coordinated signing operation.
